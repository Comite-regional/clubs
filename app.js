...
function makePopupHtml(c) {
    ...
    return `
    <div style="width: 380px; font-family: 'Inter', sans-serif; color: #1e293b; padding: 15px;">
        <div style="display:flex; align-items:center; gap:12px; border-bottom:2px solid #f1f5f9; padding-bottom:12px; margin-bottom:12px;">
            <img src="${c.logo_url || 'assets/logo_placeholder.svg'}" style="width:50px; height:50px; object-fit:contain;" onerror="this.src='assets/logo_placeholder.svg'">
            <div style="flex:1;">
        ...
        <div style="background:#f8fafc; border:1px solid #e2e8f0; border-radius:10px; padding:10px; font-size:11px;">
            <div style="font-weight:800; text-transform:uppercase; color:#475569; margin-bottom:5px;">🎯 Équipe Technique</div>
...
async function init() {
    ...
    try {
        ...
        clubs = data.map((c, i) => ({ ...c, id: c.id || i, lat: Number(c.lat), lng: Number(c.lon || c.lng) }));
